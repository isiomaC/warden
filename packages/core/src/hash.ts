...
function _jsSha256(data: Uint8Array): string {
  let h0 = 0x6a09e667,
    h1 = 0xbb67ae85,
    h2 = 0x3c6ef372,
    h3 = 0xa54ff53a;
...
function _padMessage(data: Uint8Array): Uint8Array {
  const ml = data.length * 8;
  const padding = 64 - ((data.length + 9) % 64);
  const total = data.length + padding + 8;
